...
public class BlockJukeBox extends Block {
    ...
    public void breakBlock(World world, int x, int y, int z, int id, int meta)
    {
        ...
        {
            ((TileJukeBox) world.getBlockTileEntity(x, y, z)).stopPlayingRecord();
        }
    ...
    private void dropInventory(World world, int x, int y, int z)
    {
        ...
        for (int i = 0; i < inventory.getSizeInventory(); i++)
        {
            ...
            if (itemStack != null && itemStack.stackSize > 0)
            {
                ...
                final EntityItem entityItem = new EntityItem(world, x + dX, y + dY, z + dZ, new ItemStack(itemStack.itemID, itemStack.stackSize,
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9)
    {
        ...
        else
        {
            if (!world.isRemote)
            {
                final TileJukeBox tileJukeBox = (TileJukeBox) world.getBlockTileEntity(x, y, z);

                if (tileJukeBox != null)
                {
                    player.openGui(JukeboxReloaded.instance(), GuiHandler.JUKEBOX, world, x, y, z);
                }
                else
                {
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, int blockID)
    {
        if (!world.isRemote)
        {
            ...
            // final PacketCustom packet = new
            // PacketCustom(Reference.CHANNEL_NAME, 1);
