 ...
 */
public abstract class TAsynchronousFilteredAudioInputStream extends
		TAudioInputStream implements TCircularBuffer.Trigger {
	private static final int		DEFAULT_BUFFER_SIZE		= 327670;
	private static final int		DEFAULT_MIN_AVAILABLE	= 4096;
	private static final byte[]		EMPTY_BYTE_ARRAY		= new byte[0];
	
	private final TCircularBuffer	m_circularBuffer;
	 ...
	 */
	public TAsynchronousFilteredAudioInputStream(AudioFormat outputFormat,
			long lLength, int nBufferSize, int nMinAvailable) {
		/*
	...
	public int read() throws IOException {
		...
		final int nReturn = read(m_abSingleByte);
		if (nReturn == -1) nByte = -1;
		else // $$fb 2001-04-14 nobody really knows that...
	...
	public int read(byte[] abData) throws IOException {
		...
		final int nRead = read(abData, 0, abData.length);
	...
	public int read(byte[] abData, int nOffset, int nLength) throws IOException {
		...
		final int nRead = m_circularBuffer.read(abData, nOffset, nLength);
	 ...
	 * the minimum available property, more writing is encouraged, so this
	 * method
