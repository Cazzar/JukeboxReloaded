...
public final class Header {
	...
	public static final String	bitrate_str[][][]		= {
			...
			"144 kbit/s", "160 kbit/s", "176 kbit/s", "192 kbit/s",
			...
			"56 kbit/s", "64 kbit/s", "80 kbit/s", "96 kbit/s", "112 kbit/s",
			"128 kbit/s", "160 kbit/s", "192 kbit/s", "224 kbit/s",
	 ...
	 */
	public int bitrate() {
		if (h_vbr == true) return ((int) ((h_vbr_bytes * 8) / (ms_per_frame() * h_vbr_frames))) * 1000;
	...
	public String bitrate_string() {
		if (h_vbr == true) return Integer.toString(bitrate() / 1000) + " kb/s";
	...
	public int calculate_framesize() {
		...
		if (h_layer == 1) {
			...
			// Layer III slots
			if (h_layer == 3) {
				if (h_version == MPEG1) nSlots = framesize
						- ((h_mode == SINGLE_CHANNEL) ? 17 : 32) // side info
																	// size
						- ((h_protection_bit != 0) ? 0 : 2) // CRC size
						- 4; // header size
				else nSlots = framesize - ((h_mode == SINGLE_CHANNEL) ? 9 : 17) // side
																				// info
																				// size
						- ((h_protection_bit != 0) ? 0 : 2) // CRC size
						- 4; // header size
	...
	public int max_number_of_frames(int streamsize) // E.B
	{
		...
		else if ((framesize + 4 - h_padding_bit) == 0) return 0;
	...
	{
		if (h_vbr == true) return h_vbr_frames;
		else if ((framesize + 5 - h_padding_bit) == 0) return 0;
	...
	public float ms_per_frame() // E.B
	{
		if (h_vbr == true) {
			...
			final float ms_per_frame_array[][] = { { 8.707483f, 8.0f, 12.0f },
	...
	void parseVBR(byte[] firstframe) throws BitstreamException {
		...
		else if (h_mode == SINGLE_CHANNEL) offset = 13 - 4;
		else offset = 21 - 4;
		try {
	...
	void read_header(Bitstream stream, Crc16[] crcp) throws BitstreamException {
		...
		do {
			...
			if (h_layer == 1) h_number_of_subbands = 32;
				...
				if ((channel_bitrate == 1) || (channel_bitrate == 2)) if (h_sample_frequency == THIRTYTWO) h_number_of_subbands = 12;
				else h_number_of_subbands = 8;
				else if ((h_sample_frequency == FOURTYEIGHT)
						|| ((channel_bitrate >= 3) && (channel_bitrate <= 5))) h_number_of_subbands = 27;
			...
			if ((framesize >= 0) && (framesizeloaded != framesize)) // Data
																	// loaded
																	// does not
																	// match to
																	// expected
				...
				throw stream
						.newBitstreamException(BitstreamErrors.INVALIDFRAME);
