/*
 * 09/26/08 throw exception on subbband alloc error: Christopher G. Jennings
 * (cjennings@acm.org)
...
class LayerIDecoder implements FrameDecoder {
	...
	static class SubbandLayer1 extends Subband {
	   ...
	   */
		...
		public boolean put_next_sample(int channels, SynthesisFilter filter1,
			...
			if ((allocation != 0) && (channels != OutputChannels.RIGHT_CHANNEL)) {
				final float scaled_sample = (sample * factor + offset)
						* scalefactor;
	   ...
	   */
		...
		public void read_allocation(Bitstream stream, Header header, Crc16 crc)
			...
			if ((allocation = stream.get_bits(4)) == 15) // CGJ: catch this
															// condition and
															// throw appropriate
															// exception
		...
		public boolean put_next_sample(int channels, SynthesisFilter filter1,
			...
			if (allocation != 0) {
				...
				if (channels == OutputChannels.BOTH_CHANNELS) {
					final float sample1 = sample * scalefactor, sample2 = sample
		...
		public boolean put_next_sample(int channels, SynthesisFilter filter1,
			...
			if ((channel2_allocation != 0)
				...
				final float sample2 = (channel2_sample * channel2_factor + channel2_offset)
						* channel2_scalefactor;
				if (channels == OutputChannels.BOTH_CHANNELS) filter2
						.input_sample(sample2, subbandnumber);
	...
	protected Crc16				crc	= null;		// new Crc16[1] to enable
													// CRC checking.
	...
	protected void createSubbands() {
		int i;
		if (mode == Header.SINGLE_CHANNEL) for (i = 0; i < num_subbands; ++i)
