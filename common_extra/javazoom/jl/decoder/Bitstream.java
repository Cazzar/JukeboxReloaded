...
public final class Bitstream implements BitstreamErrors {
	...
	public InputStream getRawID3v2() {
		if (rawid3v2 == null) return null;
			...
			final ByteArrayInputStream bain = new ByteArrayInputStream(rawid3v2);
			return bain;
		}
	}
	...
	public boolean isSyncCurrentPosition(int syncmode)
		...
		final int headerstring = ((syncbuf[0] << 24) & 0xFF000000)
	...
	public boolean isSyncMark(int headerstring, int syncmode, int word) {
		...
		if (syncmode == INITIAL_SYNC) // sync = ((headerstring & 0xFFF00000) ==
										// 0xFFF00000);
		sync = ((headerstring & 0xFFE00000) == 0xFFE00000); // SZD: MPEG 2.5
		else sync = ((headerstring & 0xFFF80C00) == word)
	...
	public Header readFrame() throws BitstreamException {
		...
		try {
			...
			if ((ex.getErrorCode() == INVALIDFRAME)) // Try to skip this frame.
			...
			try {
				...
				if ((e.getErrorCode() != STREAM_EOF)) // wrap original exception
														// so stack trace is
														// maintained.
					throw newBitstreamException(e.getErrorCode(), e);
			}
			else if ((ex.getErrorCode() != STREAM_EOF)) // wrap original
														// exception so stack
														// trace is maintained.
