 ...
 * This class does a fast downsampling from 32, 44.1 or 48 kHz to 8 kHz, if ULAW
 * is defined.
...
final class SynthesisFilter {
	...
	private float[]				actual_v;											// v1
																					// or
																					// v2
	private int					actual_write_pos;									// 0-15
	private final float[]		samples;											// 32
																					// new
																					// subband
																					// samples
	...
	static private float[] load_d() {
		try {
			...
			final Object o = JavaLayerUtils.deserializeArrayResource("sfd.ser",
					elemType, 512);
			return (float[]) o;
		}
	...
	private void compute_new_v() {
		...
		final int pos = actual_write_pos;
	...
	private void compute_pcm_samples0(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[0 + dvp] * dp[0]) + (vp[15 + dvp] * dp[1])
	...
	private void compute_pcm_samples1(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[1 + dvp] * dp[0]) + (vp[0 + dvp] * dp[1])
