...
public class CodecJLayerMP3 implements ICodec {
	...
	private static byte[] appendByteArrays(byte[] arrayOne, byte[] arrayTwo,
			int length) {
		byte[] newArray;
		if (arrayOne == null && arrayTwo == null) // no data, just return
		return null;
	...
	public boolean initialize(URL url) {
		...
		try {
			...
			int channels;
			if (mainHeader.mode() < 3) channels = 2;
			else channels = 1;
			...
			myAudioFormat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED,
					mainHeader.frequency(), 16, channels, channels * 2,
					mainHeader.frequency(), false);
			
			final AudioFormat mpegAudioFormat = new AudioFormat(
					AudioFormat.Encoding.PCM_SIGNED, -1.0f, 16, channels,
					channels * 2, -1.0f, false);
			
			myAudioInputStream = new DecodedMpegAudioInputStream(myAudioFormat,
					new AudioInputStream(new BufferedInputStream(
							url.openStream()), mpegAudioFormat, -1));
		}
		catch (final Exception e) {
	...
	public SoundBuffer read() {
		...
		final AudioFormat audioFormat = myAudioInputStream.getFormat();
		...
		final byte[] streamBuffer = new byte[SoundSystemConfig
		...
		final SoundBuffer buffer = new SoundBuffer(streamBuffer, audioFormat);
	...
	public SoundBuffer readAll() {
		...
		final int fileSize = myAudioFormat.getChannels()
		...
		final SoundBuffer soundBuffer = new SoundBuffer(fullBuffer,
				myAudioFormat);
