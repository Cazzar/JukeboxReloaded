...
/**
 * The interface passed to upgrades by turtles, providing methods that they can call. This should not be implemented by
...
public interface ITurtleAccess {
    ...
    /**
     * Returns a vector containing the co-ordinates at which the turtle is rendered. This will shift when the turtle is
     * moving.
     *
    ...
    /**
     * Returns which slot the turtle currently has selected in its inventory using turtle.select(). Unlike the 1-based lua
     * representation, this will be between 0 and getInventorySize() - 1.
     *
    ...
    /**
     * Tries to store an item stack into the turtles current inventory, starting from the turtles currently selected
     * inventory slot.
     *
     * @param stack The item stack to try and store.
     *
     * @return true if the stack was completely stored in the inventory, false if it was only partially stored, or could
     *         not fit at all. If false is returned and the stack was partially stored, the ItemStack passed into "stack"
     *         will now represent the stack of items that is left over.
     */
    public boolean storeItemStack(net.minecraft.item.ItemStack stack);

    /**
     * Drops an item stack from the turtle onto the floor, or into an inventory is there is one adjacent to the turtle in
     * the direction specified.
     *
     * @param stack The item stack to drop.
     * @param dir   The world direction to drop the item
     *
     * @return true if the stack was dropped, or completely stored in the adjacent inventory, false if it was only
     *         partially stored in the adjacent inventory, or could not fit at all. If false is returned and the stack was
     *         partially stored, the ItemStack passed into "stack" will now represent the stack of items that is left
     *         over.
     */
    public boolean dropItemStack(net.minecraft.item.ItemStack stack, int dir);

    /**
     * "Deploys" an item stack in the direction specified. This simulates a player right clicking, and calls onItemUse() on
     * the Item class. Will return true if some kind of deployment happened, and may modify the item stack. For block item
     * types, this can be used to place blocks. Some kinds of items (such as shears when facing a sheep) may modify the
     * turtles inventory during this call.
     *
    ...
    /**
     * Tries to "attack" entities with an item stack in the direction specified. This simulates a player left clicking, but
     * will not affect blocks. If an entity is attacked and killed during this call, its dropped items will end up in the
     * turtles inventory.
     *
    ...
    /**
     * Returns the current fuel level of the turtle, this is the same integer returned by turtle.getFuelLevel(), that
     * decreases by 1 every time the turtle moves. Can be used to have your tool or peripheral require or supply fuel to
     * the turtle.
     *
    ...
    /**
     * Removes some fuel from the turtles fuel supply. Negative numbers can be passed in to INCREASE the fuel level of the
     * turtle.
     *
     ...
     * executed on the main thread, so are guaranteed to be able to access Minecraft objects safely, and will be queued up
     * with the turtles standard movement and tool commands. An issued command will return an unique integer, which will be
     * supplied as a parameter to a "turtle_response" event issued to the turtle after the command has completed. Look at
     * the lua source code for "rom/apis/turtle" for how to build a lua wrapper around this functionality.
     *
